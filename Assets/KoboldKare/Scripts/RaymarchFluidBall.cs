    ...
    private void HandleCollision(Collision collision) {
        if (Time.timeSinceLevelLoad - lastTime > mixInterval) {
            ...
            if (spilled.ContainsKey(ReagentData.ID.Water) && spilled[ReagentData.ID.Water].volume > spilled.volume*0.9f) {
                GameManager.instance.SpawnDecalInWorld(splatterMaterial, collision.contacts[0].point + norm*0.25f, -norm, Vector2.one*Mathf.Max(spilled.volume*70f,0.6f), c, collision.contacts[0].otherCollider.gameObject, 0.5f, false, true, true);
            } else {
                GameManager.instance.SpawnDecalInWorld(splatterMaterial, collision.contacts[0].point + norm*0.25f, -norm, Vector2.one*Mathf.Max(spilled.volume*70f,0.6f), c, collision.contacts[0].otherCollider.gameObject, 0.5f, false, true, false);
            }
        }
    }
