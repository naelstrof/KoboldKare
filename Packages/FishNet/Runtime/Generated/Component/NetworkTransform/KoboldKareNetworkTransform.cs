...
namespace FishNet.Component.Transforming
{
    ...
    public sealed class KoboldKareNetworkTransform : NetworkBehaviour {
        #region Types.
        [Serializable]
        ...
        public override void OnStartNetwork() {
            _cachedTransform = _componentConfiguration switch {
                ComponentConfigurationType.Disabled => transform,
                ComponentConfigurationType.CharacterController => GetComponentInChildren<CharacterController>()?.transform ?? transform,
                ComponentConfigurationType.Rigidbody => GetComponentInChildren<Rigidbody>()?.transform ?? transform,
                ComponentConfigurationType.Rigidbody2D => GetComponentInChildren<Rigidbody2D>()?.transform ?? transform,
                _ => transform
            };
        ...
        private void ConfigureComponents()
        {
            ...
            {
                if (_cachedTransform.TryGetComponent(out Rigidbody c))
                {
            ...
            {
                if (_cachedTransform.TryGetComponent(out Rigidbody2D c))
                {
                    //If first time set starting interpolation.
            ...
            {
                if (_cachedTransform.TryGetComponent(out CharacterController c))
                {
                    //Client auth.
                    if (_clientAuthoritative)
                    {
